package server

import (
	"newsportal/pkg/newsportal"
)

func TagsToNews(in *NewsSummary, tags []newsportal.Tag) (out NewsSummary) {
	for _, tag := range tags {
		newTag := TagFromManager(&tag)
		in.Tags = append(in.Tags, *newTag)
	}
	return
}
func CategoryToNews(in *NewsSummary, category *newsportal.Category) (out NewsSummary) {
	newCategory := CategoryFromManager(category)
	in.Category = *newCategory
	return
}

func NewsSummaryFromManager(in *newsportal.News, tags []newsportal.Tag) (out *NewsSummary) {
	if in != nil {
		out = &NewsSummary{
			ID:          in.ID,
			Title:       in.Title,
			Foreword:    in.Foreword,
			Content:     in.Content,
			Author:      in.Author,
			PublishedAt: in.PublishedAt,
		}
		TagsToNews(out, tags)
		CategoryToNews(out, in.Category)
	}
	return
}

func ShortNewsFromManager(in *newsportal.News, tags []newsportal.Tag) (out *News) {
	if in != nil {
		out = &News{
			ID:          in.ID,
			Title:       in.Title,
			Foreword:    in.Foreword,
			Author:      in.Author,
			PublishedAt: in.PublishedAt,
		}
		ShortTagsToNews(out, tags)
		ShortCategoryToNews(out, in.Category)
	}
	return
}

func ShortTagsToNews(in *News, tags []newsportal.Tag) (out NewsSummary) {
	for _, tag := range tags {
		newTag := TagFromManager(&tag)
		in.Tags = append(in.Tags, *newTag)
	}
	return
}
func ShortCategoryToNews(in *News, category *newsportal.Category) (out NewsSummary) {
	newCategory := CategoryFromManager(category)
	in.Category = *newCategory
	return
}

func CategoryFromManager(in *newsportal.Category) (out *Category) {
	if in != nil {
		out = &Category{
			ID:          in.ID,
			Title:       in.Title,
			OrderNumber: in.OrderNumber,
			Alias:       in.Alias,
		}
	}
	return
}

func CategoriesFromManager(in []newsportal.Category) (out []Category) {
	for _, category := range in {
		newCategories := CategoryFromManager(&category)
		out = append(out, *newCategories)
	}
	return
}

func TagFromManager(in *newsportal.Tag) (out *Tag) {
	if in != nil {
		out = &Tag{
			ID:    in.ID,
			Title: in.Title,
		}
	}
	return
}

func TagsFromManager(in []newsportal.Tag) (out []Tag) {
	for _, tag := range in {
		newTags := TagFromManager(&tag)
		out = append(out, *newTags)
	}
	return
}
